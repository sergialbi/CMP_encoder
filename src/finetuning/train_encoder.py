...
from transformers import BertTokenizer, BertForPreTraining, AutoTokenizer
...
def train(model: BertClassifier, tokenizer:AutoTokenizer, labels_dict:dict, train_data:pd.DataFrame, val_data:pd.DataFrame, epochs:int, criterion, optimizer, bs:int=2, patience=5):
    """
...
def evaluate(model: BertClassifier, tokenizer:AutoTokenizer, labels_dict:dict, test_data:pd.DataFrame):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-path", type=str, default="/home/bsc/bsc048726/politics/new_cmp_encoder/datasets/cmp/national/v5", help="The path to the dataset file for training")
    parser.add_argument("--train-file-name", type=str, default="CMPD_c1_train.xlsx", help="The name of the training file")
    parser.add_argument("--tokenizer", type=str, default='/home/bsc/bsc048726/politics/new_cmp_encoder/models/EuroBERT/EuroBERT-210m', help="The BERT tokenizer and pre-trained weights name (download) or path")
    parser.add_argument("--save-model-path", type=str, default="/home/bsc/bsc048726/politics/new_cmp_encoder/results/finetuning", help="The path to store the model")
    parser.add_argument('--use-pretrained-model', action='store_true', help="Use a pretrained model")
    parser.add_argument("--pretrained-model-path", type=str, default="/home/bsc/bsc048726/politics/new_cmp_encoder/results/intermediate_pretrain/model_pretrained", help="The path where the pretrained model is")
    ...
    parser.add_argument("--results-path", type=str, default="/home/bsc/bsc048726/politics/new_cmp_encoder/results/finetuning/", help="The path to store the figures")
    ...
    val_path = os.path.join(args.data_path, "CMPDb_val.csv")
    test_path = os.path.join(args.data_path, "CMPDb_test.csv")
    ...
    print("1.1", type(tokenizer), tokenizer)
    if args.use_pretrained_model:
        model = BertClassifier(tokenizer_name = args.tokenizer, num_classes = num_classes, pretrained_model_path=args.pretrained_model_path)
    else:
    ...
    if args.reduced: 
        ...
        print(f"Reducing training dataset size to: {n_samples_train}")
    ...
    pretrained = "_pretrained" if args.use_pretrained_model else ""
    name = args.train_file_name.split(".")[0]+pretrained
    if args.print_graphs:
        file_name=args.data_path.split("/")[-1].split(".")[0]
        title=f"{file_name} metrics accuracy"
        print_results(train_acc, train_loss, val_acc, val_loss, args.results_path, title, extra_save = f"_{name}")
        title=f"{file_name} metrics f1"
        print_results_f1(train_f1, train_loss, val_f1, val_loss, args.results_path, title, extra_save = f"_{name}")
    
    print("Saving model...")
    save_path = os.path.join(args.save_model_path, f"torchmodel{pretrained}_weights_{name}.pth")
